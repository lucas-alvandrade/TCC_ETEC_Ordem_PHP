...
class Home_model extends CI_Model {
    ...
    public function get_contas_receber_vencem_hoje() {
        ...
        $this->db->select([
            'contas_receber.*',
            'cliente_id',
            'CONCAT (clientes.cliente_name, " ", clientes.cliente_sobrenome) as cliente_nome_completo',
        ]);
    ...
    public function get_produtos_mais_vendidos() {
        ...
        $this->db->limit(3); //Quantos produtos irão retornar
    ...
    public function get_total_servicos_ativos() {

        $this->db->select([
            'COUNT(ordem_servico_id) as quantidade_ordens_servicos',
        ]);

        $this->db->where('ordem_servico_status', 0);

        return $this->db->get('ordens_servicos')->row();
    }

    public function get_servicos_ativos() {
        ...
        $this->db->where('ordem_servico_status', 0);
        $this->db->limit(3); //Quantas ordens irão retornar

        $this->db->order_by('ordem_servico_data_emissao');
